...
use notifications::*;
use squs_common::activity_pub::inbox::AsActor;
use posts::Post;
use schema::mentions;
use users::User;
use PlumeRocket;
...
impl Mention {
    ...
    pub fn build_activity(c: &PlumeRocket, ment: &str) -> Result<link::Mention> {
        ...
        mention.link_props.set_href_string(user.ap_id)?;
    ...
    pub fn to_activity(&self, conn: &Connection) -> Result<link::Mention> {
        ...
        let mut mention = link::Mention::default();
        mention.link_props.set_href_string(user.ap_id.clone())?;
        mention
            .link_props
    ...
    pub fn from_activity(
        ...
        let ap_id = ment.link_props.href_string().ok()?;
        let mentioned = User::find_by_ap_id(conn, &ap_id)?;
