...
use schema::users;
use {ap_id, Connection, Error, PlumeRocket, Result};
...
impl User {
    insert!(users, NewUser, |inserted, conn| {
        ...
        if inserted.ap_id.is_empty() {
            inserted.ap_id = instance.compute_box(USER_PREFIX, &inserted.username, "");
        }

        if inserted.shared_inbox_url.is_none() {
            inserted.shared_inbox_url = Some(ap_id(&format!("{}/inbox", instance.public_domain)));
        }
    ...
    find_by!(users, find_by_ap_id, ap_id as &str);

    pub fn get_token(&self, conn: &Connection) -> Result<String> {
        crate::api_tokens::ApiToken::find_for_user(conn, self.id).map(|t| t.value)
    }
    ...
    pub fn refetch(&self, conn: &Connection) -> Result<()> {
        User::fetch(&self.ap_id.clone()).and_then(|json| {
            diesel::update(self)
                .set((
                    ...
                    users::avatar_url.eq(json.object.object_props.icon_image().ok().and_then(|img| img.object_props.url_string().ok())),
    ...
    pub fn has_liked(&self, conn: &Connection, comment: &Comment) -> Result<bool> {
        use schema::likes;
        likes::table
            .filter(likes::comment_id.eq(comment.id))
    ...
    pub fn has_reshared(&self, conn: &Connection, comment: &Comment) -> Result<bool> {
        use schema::reshares;
        reshares::table
            .filter(reshares::comment_id.eq(comment.id))
    ...
    pub fn to_activity(&self) -> Result<CustomPerson> {
        let mut actor = Person::default();
        actor.object_props.set_id_string(self.ap_id.clone())?;
        actor
            .object_props
        ...
        actor.object_props.set_url_string(self.ap_id.clone())?;
        actor
            .ap_actor_props
        ...
        public_key.set_id_string(format!("{}#main-key", self.ap_id))?;
        public_key.set_owner_string(self.ap_id.clone())?;
        ...
        avatar.object_props.set_url_string(self.avatar_url.clone().unwrap_or_default())?;
    ...
    pub fn delete_activity(&self, conn: &Connection) -> Result<Delete> {
        ...
        del.delete_props
            .set_actor_link(Id::new(self.ap_id.clone()))?;
    ...
    pub fn webfinger(&self, conn: &Connection) -> Result<Webfinger> {
        Ok(Webfinger {
            ...
            aliases: vec![self.ap_id.clone()],
            links: vec![
                Link {
                    rel: String::from("http://webfinger.net/rel/profile-page"),
                    mime_type: Some(String::from("text/html")),
                    href: Some(self.ap_id.clone()),
                    template: None,
                },
                Link {
    ...
    pub fn set_avatar(&self, conn: &Connection, url: String) -> Result<()> {
        diesel::update(self)
    ...
    fn from_activity(c: &PlumeRocket, acct: CustomPerson) -> Result<Self> {
        ...
        let user = User::insert(
            ...
            NewUser {
                ...
                ap_id: acct.object.object_props.id_string()?,
                public_key: acct
                    .custom_props
                    .public_key_publickey()?
                    .public_key_pem_string()?,
...
impl NewUser {
    ...
    pub fn new_local(
        ...
        let app = crate::apps::App::insert(conn, crate::apps::NewApp {
            name: "Squs comments".into(),
            client_id: squs_common::utils::random_hex(),
            client_secret: squs_common::utils::random_hex(),
            redirect_uri: None,
            website: None,
        })?;
        crate::api_tokens::ApiToken::insert(conn, crate::api_tokens::NewApiToken {
            app_id: app.id,
            user_id: res.id,
            value: squs_common::utils::random_hex(),
            scopes: "read:comments+write:posts".into(),
        })?;
        Ok(res)
    }
}
...
pub(crate) mod tests {
    ...
    fn self_federation() {
        ...
        conn.test_transaction::<_, (), _>(|| {
            ...
            assert_eq!(user.ap_id, users[0].ap_id);
