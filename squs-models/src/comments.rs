...
impl Comment {
    ...
    find_by!(comments, find_by_ap_id, ap_id as &str);
    ...
    pub fn to_activity(&self, c: &PlumeRocket) -> Result<Note> {
        ...
        note.object_props
            .set_id_string(self.ap_id.clone())?;
        note.object_props
        ...
        note.object_props
            .set_in_reply_to_link(Id::new(self.in_response_to_id.map_or_else(
                || Ok(Post::get(&c.conn, self.post_id)?.ap_id),
                |id| Ok(Comment::get(&c.conn, id)?.ap_id) as Result<String>,
            )?))?;
        note.object_props
    ...
    pub fn notify(&self, conn: &Connection) -> Result<()> {
        let author = self.get_post(conn)?.get_author(conn)?;
...
impl FromId<PlumeRocket> for Comment {
    ...
    fn from_activity(c: &PlumeRocket, note: Note) -> Result<Self> {
        ...
        let comm = {
            ...
            let previous_comment = Comment::find_by_ap_id(conn, previous_url);
            ...
            if let Some(serde_json::Value::Array(tags)) = note.object_props.tag.clone() {
                for tag in tags {
                    serde_json::from_value::<link::Mention>(tag)
                        .map_err(Error::from)
                        .and_then(|m| {
                            let author = &Post::get(conn, comm.post_id)?.get_author(conn)?;
                            let not_author = m.link_props.href_string()? != author.ap_id.clone();
        ...
        if !comm.public_visibility {
            let receivers_ap_id = |v: Option<serde_json::Value>| {
            ...
            let to = receivers_ap_id(note.object_props.to.take());
            let cc = receivers_ap_id(note.object_props.cc.take());
            let bto = receivers_ap_id(note.object_props.bto.take());
            let bcc = receivers_ap_id(note.object_props.bcc.take());

            let receivers_ap_id = to
                .chain(cc)
                .chain(bto)
                .chain(bcc)
        ...
        conn.test_transaction::<_, (), _>(|| {
            ...
            match inbox(&r, serde_json::to_value(act).unwrap()).unwrap() {
                InboxResult::Commented(c) => {
                    ...
                    assert_eq!(c.ap_id, original_comm.ap_id);
