...
use schema::follows;
use users::User;
use {ap_id, Connection, Error, PlumeRocket, Result, CONFIG};
...
impl Follow {
    insert!(
        follows,
        NewFollow,
        |inserted, conn| if inserted.ap_id.is_empty() {
            inserted.ap_id = ap_id(&format!("{}/follows/{}", CONFIG.base_url, inserted.id));
    ...
    get!(follows);
    find_by!(follows, find_by_ap_id, ap_id as &str);
    ...
    pub fn accept_follow<A: Signer + IntoId + Clone, B: Clone + AsActor<T> + IntoId, T>(
        ...
        let res = Follow::insert(
            ...
            NewFollow {
                ...
                ap_id: follow.object_props.id_string()?,
            },
        )?;
        res.notify(conn)?;

        let mut accept = Accept::default();
        let accept_id = ap_id(&format!(
            "{}/follow/{}/accept",
    ...
    pub fn build_undo(&self, conn: &Connection) -> Result<Undo> {
        ...
        undo.object_props
            .set_id_string(format!("{}/undo", self.ap_id))?;
        undo.undo_props
...
impl FromId<PlumeRocket> for Follow {
    ...
    fn from_db(c: &PlumeRocket, id: &str) -> Result<Self> {
        Follow::find_by_ap_id(&c.conn, id)
    }
        ...
        conn.test_transaction::<_, (), _>(|| {
            ...
            let follow = Follow::insert(
                ...
                NewFollow {
                    ...
                    ap_id: String::from("https://some.url/"),
                },
            )
            .expect("Couldn't insert new follow");
            assert_eq!(follow.ap_id, String::from("https://some.url/"));
            Ok(())
        });
    }
}
